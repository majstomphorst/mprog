...
void init(void)
{
    // keeping track of with space neede for function move
    w_y = d - 1;
    w_x = d - 1;
...
{
    // printing the board 
    // select first y as than print x as and move to next y
    for (int y = 0; y < d; y++)
    {
        for (int x = 0; x < d; x++)
        {
...
bool move(int tile)
{
    ...
    {
        // swap
        // give wihte tile the value that the user gave (tile)
        board[w_y][w_x] = tile;
        // give the place of (old) tile 0 (wihte)
        board[w_y - 1][w_x] = 0;
